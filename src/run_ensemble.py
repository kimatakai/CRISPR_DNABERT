...
# Rest of Parser
parser.add_argument("--models", "-m", type=str, required=False, default="GRU-Embed,CRISPR-BERT,CRISPR-HW,CRISPR-DIPOFF,CrisprBERT,DNABERT-Epi", help="Models.")
...
SetConfigClass = check_set.SetPathsEnsembleResult(config)
config = SetConfigClass.set_path_ensemble()


def main() -> None:
    print("Start ensemble result generation...")
    ResultsForEnsembleClass = result_module.ResultsForEnsemble(config)
    ResultsForEnsembleClass.save_ensemble_results()
    ResultsForEnsembleClass.create_workbook()
    ResultsForEnsembleClass.add_ensemble_result_sheet()
    ResultsForEnsembleClass.plot_ensemble_importance()
