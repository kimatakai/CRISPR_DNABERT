...
label_tokens = sorted(set([''.join(combination) for combination in product("ATGC", repeat=2)]))
label_tokens_sub = ["--", "-A", "-T", "-G", "-C", "A-", "T-", "G-", "C-", "NA", "NT", "NG", "NC", "AN", "TN", "GN", "CN"]
label2id = {label: idx for idx, label in enumerate(label_tokens)}
for l in label_tokens_sub:
    label2id[l] = len(label_tokens) + 1
...
def token_to_onehot(token):
    onehot = torch.zeros(len(label_tokens)+1)
    if token in label_tokens:
        index = label2id[token]
        onehot[index] = 1.0
    return onehot


def return_pair_tokens(seq1, seq2):
    if len(seq1) != len(seq2):
        raise ValueError("The sequences must be of the same length.")
    
    token_labels_one_hot = []
    # k-merでseq1とseq2をループで取り出し、例）seq1='ATGCA'、seq2='ATGCG'の場合、["ATGATG", "TGCTGC", "GCAGCG"]みたいにペアのトークンを取り出す。
    for i in range(len(seq1)):
        seq1_kmer = seq1[i]
        seq2_kmer = seq2[i]
        
        token_labels_one_hot.append(token_to_onehot(seq1_kmer+seq2_kmer))
    token_labels_one_hot = torch.concat(token_labels_one_hot, dim=0)
    
    return token_labels_one_hot
...
def generate_dna_samples(n_samples=10):
    ...
    for mismatch_count in range(7):  # Mismatch counts from 0 to 6
        for _ in range(n_samples // 7):  # Ensure each mismatch count is equally represented
            ...
            pair_token = return_pair_tokens(dna, sgrna)
    ...
    shuffled_pair_tokens = [pair_tokens[i] for i in indices]
    
    datasets = Dataset.from_dict({'targetDNA' : shuffled_dna_samples, 'sgRNA' : shuffled_sgrna_samples, 'label' : shuffled_pair_tokens}) 
...
def main():
    ...
    # create train data
    datasets = generate_dna_samples(n_samples=500000) # 500000
    ...
    print(f'label [0]           : ({len(datasets["label"][0])})')
    ...
    if from_pretrain:
        model = AutoModelForSequenceClassification.from_pretrained(
            ...
            num_labels=24*(len(label_tokens)+1),
        ...
        config.num_labels = 24*(len(label_tokens)+1)
